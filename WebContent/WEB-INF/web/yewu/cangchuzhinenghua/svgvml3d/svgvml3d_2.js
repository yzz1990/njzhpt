...
function SVGObject(ii, oo) {
	this.idx = ii;
	this.GetSVGDoc = _GetSVGDoc;
	this.GetScene = _GetScene;
	this.SVGDocument = oo;
}
function AddSVGObject(oo) {
	var ll = SVGObjects.length;
	SVGObjects[ll] = new SVGObject(ll, oo);
}
function _GetSVGDoc() {
...
function _Scene3DInit() {
	if (useSVG) {
		this.BoxGroup = this.Parent.GetSVGDoc().createElementNS(
		...
		this.PolyGroup = this.Parent.GetSVGDoc().createElementNS(
				"http://www.w3.org/2000/svg", "g");
		...
		this.BoxGroup.coordsize = parseInt(this.XM * 2) + ","
				+ parseInt(this.YM * 2);
		...
		this.PolyGroup.coordsize = parseInt(this.XM * 2) + ","
				+ parseInt(this.YM * 2);
...
function _Scene3DChangeLight(ddTh, ddFi) {
	...
	if ((this.ThLight + ddTh >= -89.0) && (this.ThLight + ddTh <= 89.0))
		this.ThLight += ddTh;
	this.FiLight += ddFi;
	if (this.ThLight < -89.0)
		this.ThLight = -89.0;
	if (this.ThLight > 89.0)
		this.ThLight = 89.0;
	while (this.FiLight < 0.0)
		this.FiLight += 360.0;
	while (this.FiLight >= 360.0)
		this.FiLight -= 360.0;
	this.Light.x = Math.cos(this.FiLight * pi_d_180)
			* Math.cos(this.ThLight * pi_d_180);
	this.Light.y = -Math.sin(this.FiLight * pi_d_180)
			* Math.cos(this.ThLight * pi_d_180);
...
function _Scene3DAddPoly(oo) {
	...
	if (useSVG) {
		this.Shape[ii] = this.Parent.GetSVGDoc().createElementNS(
...
function _Scene3DAutoCenter() {
	...
	if (vv > 0.0)
		...
		ll = 19.0;
	if (this.XM < this.YM)
		this.ZoomAll = 1.6 * this.XM / ll;
	else
		this.ZoomAll = 1.6 * this.YM / ll;
	this.Dist = 2 * ll;
	this.ChangeViewer(0, 0);
}
function _Scene3DScreenPos(vv) {
	nn = new Vector(0, 0, 0);
	nn.x = this.sin_Fi * (vv.x - this.Center.x) + this.cos_Fi
			* (vv.y - this.Center.y);
	nn.y = -this.cos_Fi_sin_Th * (vv.x - this.Center.x) + this.sin_Fi_sin_Th
			* (vv.y - this.Center.y) - this.cos_Th * (vv.z - this.Center.z);
	nn.z = this.cos_Fi_cos_Th * (vv.x - this.Center.x) - this.sin_Fi_cos_Th
			* (vv.y - this.Center.y) - this.sin_Th * (vv.z - this.Center.z);
	if (this.Dist > 0.0) {
...
function _Scene3DSort() {
	...
	if (this.Dist == 0.0) {
		for (ii = 0; ii < ll; ii++) {
			...
			this.PolyRank[ii][1] = this.cos_Fi_cos_Th * this.Poly[ii].Center.x
					* this.OrderWeight.x - this.sin_Fi_cos_Th
					* this.Poly[ii].Center.y * this.OrderWeight.y - this.sin_Th
					* this.Poly[ii].Center.z * this.OrderWeight.z;
		}
	} else {
		for (ii = 0; ii < ll; ii++) {
			this.PolyRank[ii][0] = ii;
...
function _Scene3DDraw() {
	...
	if (useSVG) {
		// alert(this.Parent.GetScene().childNodes[6].nodeName=="text");
		if (!first) {
			this.Parent.GetScene().setAttribute("first", 1);
		}
	}
	for (ii = 0; ii < ll; ii++) {
		//alert(arraylist[this.PolyRank[ii][0]]);
		if(arraylist[this.PolyRank[ii][0]]==2000){
			arraylist[this.PolyRank[ii][0]]=0.0;
		}
		this.Shape[ii].setAttribute("text", arraylist[this.PolyRank[ii][0]]);
...
function _Scene3DGetColor(cc0, cc1, nn, pp) {
	...
	var zz, vv;
	if (this.Dist == 0.0)
		zz = -this.cos_Fi_cos_Th * nn.x + this.sin_Fi_cos_Th * nn.y
				+ this.sin_Th * nn.z;
	else
		zz = (pp.x - this.Viewer.x) * nn.x + (pp.y - this.Viewer.y) * nn.y
				+ (pp.z - this.Viewer.z) * nn.z;
	...
	if (vv <= 0) {
		...
		rr = Math
				.floor(rr * (vv * (1 - this.DiffuseLight) + this.DiffuseLight));
		gg = Math
				.floor(gg * (vv * (1 - this.DiffuseLight) + this.DiffuseLight));
		bb = Math
				.floor(bb * (vv * (1 - this.DiffuseLight) + this.DiffuseLight));
	}
	var ss = "#";
...
function Poly3D(aParentScene, aFrontColor, aBackColor, aStrokeColor,
		aStrokeWeight) {
...
function _Poly3DSetPoint(ii, xx, yy, zz) {
	...
	this.PhPoint[ii].y = yy, this.PhPoint[ii].z = zz;
	this.Point[ii].x = xx * vv.x;
	this.Point[ii].y = yy * vv.y, this.Point[ii].z = zz * vv.z;
}
...
function _Poly3DDraw(aShape) {
	...
	var ggypoing = this.Point;
	var vv = this.Parent.ScreenPos(this.Point[0]);
	if (useSVG) {
		// innerHTML
		if (this.Parent.Parent.GetScene().getAttribute("first") != 1) {
			var text2 = this.Parent.Parent.GetSVGDoc().createElementNS(
					"http://www.w3.org/2000/svg", "text");
			text2.setAttribute("width", 1);
			text2.setAttribute("height", 20);
			text2.setAttribute("font-family", "Verdana");
			text2.setAttribute("font-size", "12px");
			text2.textContent = this.text;
			aShape.parentNode.insertBefore(text2, aShape.nextSibling);
		} else {
			aShape.nextSibling.setAttribute("visibility", this.Visibility);
		}
		...
		// 设置温度字符的位置
		var weizhi = ss.split(' ');
		var xsize = [ weizhi[1], weizhi[4], weizhi[7], weizhi[10] ];
		var ysize = [ weizhi[2], weizhi[5], weizhi[8], weizhi[11] ];
		var maxleft = xsize[0];
		var maxright = xsize[0];
		var maxtop = ysize[0];
		var maxbottom = ysize[0];
		for ( var index = 0; index < 4; index++) {
			if (Number(xsize[index]) < Number(maxleft)) {
				maxleft = xsize[index];
			}
			if (Number(xsize[index]) > Number(maxright)) {
				maxright = xsize[index];
			}
			if (Number(ysize[index]) < Number(maxtop)) {
				maxtop = ysize[index];
			}
			if (Number(ysize[index]) > Number(maxbottom)) {
				maxbottom = ysize[index];
			}
		}
		// 设置文本的位置 GGY
		var text = aShape.getAttribute("text");
		aShape.nextSibling.textContent = text;
		aShape.nextSibling.setAttribute("x", Number(maxleft)
				+ (Number(maxright) - Number(maxleft)) / 2 - 15);
		aShape.nextSibling.setAttribute("y", Number(maxtop)
				+ (Number(maxbottom) - Number(maxtop)) / 2 + 5);
		// 设置背景的颜色 GGY

		if ((ll >= 3) && (this.FrontColor != ""))
			if (Number(text) <= 10)
				aShape.setAttribute("fill", this.Parent.GetColor("#00FF99",
						"#00FF99", this.Normal, this.Center));
			else if (Number(text) <= 20)
				aShape.setAttribute("fill", this.Parent.GetColor("#FFFF66",
						"#FFFF66", this.Normal, this.Center));
			else if (Number(text) <= 30)
				aShape.setAttribute("fill", this.Parent.GetColor("#FF9900",
						"#FF9900", this.Normal, this.Center));
			else if (Number(text) == 0.0)
				aShape.setAttribute("fill", this.Parent.GetColor("#A99F9F",
						"#A99F9F", this.Normal, this.Center));
			else
				aShape.setAttribute("fill", this.Parent.GetColor("#FF3300",
						"#FF3300", this.Normal, this.Center));
		else
		...
		if (this.StrokeColor)
			...
			aShape.setAttribute("stroke", this.Parent.GetColor(this.FrontColor,
					this.BackColor, this.Normal, this.Center));
		...
		for ( var jj in this.Parent.Callback) {
			...
			if (this.Callback[jj])
				aShape.addEventListener(jj, this.Callback[jj], false);
		}
	} else {
		if (this.Visibility == "visible") {
			// ����д��ȡ���¶�
			aShape.innerHTML = "<span style=\"left:35px;position:relative;top:10px;\">"
					+ this.text + "</span>";
			...
			aShape.path = ss;
			if ((ll >= 3) && (this.FrontColor != "")) {
				aShape.fillcolor = this.Parent.GetColor(this.FrontColor,
						this.BackColor, this.Normal, this.Center);
				aShape.filled = true;
			} else
				aShape.filled = false;
			if (this.StrokeColor)
				...
				aShape.strokecolor = this.Parent.GetColor(this.FrontColor,
						this.BackColor, this.Normal, this.Center);
...
function _Object3DZoom(ff) {
	for ( var ii = 0; ii < this.Poly3D.length; ii++)
		this.Poly3D[ii].Zoom(ff);
	this.Center.Zoom(ff);
}
function _Object3DShift(xx, yy, zz) {
	for ( var ii = 0; ii < this.Poly3D.length; ii++)
		this.Poly3D[ii].Shift(xx, yy, zz);
	this.Center.x += xx;
	this.Center.y += yy;
	this.Center.z += zz;
}
function _Object3DSetFrontColor(aFrontColor) {
	for ( var ii = 0; ii < this.Poly3D.length; ii++)
		this.Poly3D[ii].FrontColor = aFrontColor;
}
function _Object3DSetBackColor(aBackColor) {
	for ( var ii = 0; ii < this.Poly3D.length; ii++)
		this.Poly3D[ii].BackColor = aBackColor;
}
function _Object3DSetStrokeColor(aStrokeColor) {
	for ( var ii = 0; ii < this.Poly3D.length; ii++)
		this.Poly3D[ii].StrokeColor = aStrokeColor;
}
function _Object3DSetStrokeWeight(aStrokeWeight) {
	for ( var ii = 0; ii < this.Poly3D.length; ii++)
		this.Poly3D[ii].StrokeWeight = aStrokeWeight;
}
function _Object3DSetVisibility(isVisible) {
	for ( var ii = 0; ii < this.Poly3D.length; ii++)
		this.Poly3D[ii].Visibility = ((isVisible) && (isVisible != "hidden")) ? "visible"
				: "hidden";
}
function _Object3DRotateX(aFi, aCenter) {
	...
	for (ii = 0; ii < this.Poly3D.length; ii++) {
		for (jj = 0; jj < this.Poly3D[ii].Point.length; jj++) {
			...
			this.Poly3D[ii].Point[jj].y = aCenter * this.Center.y + ccos_Fi
					* yy - ssin_Fi * zz;
			this.Poly3D[ii].Point[jj].z = aCenter * this.Center.z + ssin_Fi
					* yy + ccos_Fi * zz;
		}
...
function _Object3DRotateY(aFi, aCenter) {
	...
	for (ii = 0; ii < this.Poly3D.length; ii++) {
		for (jj = 0; jj < this.Poly3D[ii].Point.length; jj++) {
			...
			this.Poly3D[ii].Point[jj].z = aCenter * this.Center.z + ccos_Fi
					* zz - ssin_Fi * xx;
			this.Poly3D[ii].Point[jj].x = aCenter * this.Center.x + ssin_Fi
					* zz + ccos_Fi * xx;
		}
		this.Poly3D[ii].Update();
	}
...
function _Object3DRotateZ(aFi, aCenter) {
	...
	for (ii = 0; ii < this.Poly3D.length; ii++) {
		for (jj = 0; jj < this.Poly3D[ii].Point.length; jj++) {
			xx = this.Poly3D[ii].Point[jj].x - aCenter * this.Center.x;
			yy = this.Poly3D[ii].Point[jj].y - aCenter * this.Center.y;
			this.Poly3D[ii].Point[jj].x = aCenter * this.Center.x + ccos_Fi
					* xx - ssin_Fi * yy;
			this.Poly3D[ii].Point[jj].y = aCenter * this.Center.y + ssin_Fi
					* xx + ccos_Fi * yy;
		}
		this.Poly3D[ii].Update();
	}
...
function _Object3DSetEventAction(aEvent, aCallback) {
	if (aCallback)
		this.Parent.Callback[aEvent] = aCallback;
	for ( var ii = 0; ii < this.Poly3D.length; ii++) {
		this.Poly3D[ii].Callback[aEvent] = aCallback;
	}
}

function Box3D(aParentScene, aX0, aY0, aZ0, aX1, aY1, aZ1, aFrontColor,
		aBackColor, aStrokeColor, aStrokeWeight, aMsg) {
	this.Parent = aParentScene;
	...
	this.Poly3D[0] = new Poly3D(aParentScene, aFrontColor, aBackColor,
			aStrokeColor, aStrokeWeight);
	with (this.Poly3D[0]) {
		AddPoint(aX0, aY0, aZ0);
		AddPoint(aX1, aY0, aZ0);
		AddPoint(aX1, aY1, aZ0);
		AddPoint(aX0, aY1, aZ0);
		Update();
	}
	this.Poly3D[1] = new Poly3D(aParentScene, aFrontColor, aBackColor,
			aStrokeColor, aStrokeWeight);
	with (this.Poly3D[1]) {
		AddPoint(aX0, aY0, aZ1);
		AddPoint(aX0, aY1, aZ1);
		AddPoint(aX1, aY1, aZ1);
		AddPoint(aX1, aY0, aZ1);
		Update();
	}
	this.Poly3D[2] = new Poly3D(aParentScene, aFrontColor, aBackColor,
			aStrokeColor, aStrokeWeight);
	with (this.Poly3D[2]) {
		AddPoint(aX0, aY0, aZ1);
		AddPoint(aX1, aY0, aZ1);
	...
	this.Poly3D[3] = new Poly3D(aParentScene, aFrontColor, aBackColor,
			aStrokeColor, aStrokeWeight);
	with (this.Poly3D[3]) {
		AddPoint(aX0, aY1, aZ0);
		AddPoint(aX1, aY1, aZ0);
	...
	this.Poly3D[4] = new Poly3D(aParentScene, aFrontColor, aBackColor,
			aStrokeColor, aStrokeWeight);
	with (this.Poly3D[4]) {
		AddPoint(aX0, aY1, aZ0);
		AddPoint(aX0, aY1, aZ1);
	...
	this.Poly3D[5] = new Poly3D(aParentScene, aFrontColor, aBackColor,
			aStrokeColor, aStrokeWeight);
	with (this.Poly3D[5]) {
		AddPoint(aX1, aY0, aZ0);
		AddPoint(aX1, aY0, aZ1);
...
function _Box3DSetPosition(aX0, aY0, aZ0, aX1, aY1, aZ1) {
	with (this.Poly3D[0]) {
		SetPoint(0, aX0, aY0, aZ0);
		SetPoint(1, aX1, aY0, aZ0);
		SetPoint(2, aX1, aY1, aZ0);
		SetPoint(3, aX0, aY1, aZ0);
		Update();
	}
	with (this.Poly3D[1]) {
		SetPoint(0, aX0, aY1, aZ1);
		SetPoint(1, aX1, aY1, aZ1);
		SetPoint(2, aX1, aY0, aZ1);
		SetPoint(3, aX0, aY0, aZ1);
		Update();
	}
	with (this.Poly3D[2]) {
		SetPoint(0, aX0, aY0, aZ1);
		SetPoint(1, aX1, aY0, aZ1);
		SetPoint(2, aX1, aY0, aZ0);
		SetPoint(3, aX0, aY0, aZ0);
		Update();
	}
	with (this.Poly3D[3]) {
		SetPoint(0, aX0, aY1, aZ0);
		SetPoint(1, aX1, aY1, aZ0);
		SetPoint(2, aX1, aY1, aZ1);
		SetPoint(3, aX0, aY1, aZ1);
		Update();
	}
	with (this.Poly3D[4]) {
		SetPoint(0, aX0, aY1, aZ0);
		SetPoint(1, aX0, aY1, aZ1);
		SetPoint(2, aX0, aY0, aZ1);
		SetPoint(3, aX0, aY0, aZ0);
		Update();
	}
	with (this.Poly3D[5]) {
		SetPoint(0, aX1, aY0, aZ0);
		SetPoint(1, aX1, aY0, aZ1);
		SetPoint(2, aX1, aY1, aZ1);
		SetPoint(3, aX1, aY1, aZ0);
		Update();
	}
}

function Line(aParentScene, aX0, aY0, aZ0, aX1, aY1, aZ1, aStrokeColor,
		aStrokeWeight) {
	this.Parent = aParentScene;
	this.ClassName = "Line";
	this.Center = new Vector((aX0 + aX1) / 2, (aY0 + aY1) / 2, (aZ0 + aZ1) / 2);
	...
	this.RotateZ = _Object3DRotateZ;
	this.Poly3D = new Array();
	this.Poly3D[0] = new Poly3D(aParentScene, "", "", aStrokeColor,
			aStrokeWeight);
	this.SetId = _Object3DSetId;
	this.SetEventAction = _Object3DSetEventAction;
	with (this.Poly3D[0]) {
...
function CoordSys(aParentScene, aStrokeColor) {
	...
	this.SetEventAction = _Object3DSetEventAction;
	this.Poly3D = new Array();
	this.Poly3D[0] = new Poly3D(aParentScene, "", "", this.StrokeColor,
			this.StrokeWeight);
	with (this.Poly3D[0]) {
		AddPoint(0, 0, 0);
		AddPoint(1, 0, 0);
...
function _CoordSysTransformCoord(vv) {
	vvv = new Vector(this.Poly3D[0].Point[0].x, this.Poly3D[0].Point[0].y,
			this.Poly3D[0].Point[0].z);
	vvv.x += (this.Poly3D[0].Point[1].x - this.Poly3D[0].Point[0].x) * vv.x
			+ (this.Poly3D[0].Point[2].x - this.Poly3D[0].Point[0].x) * vv.y
			+ (this.Poly3D[0].Point[3].x - this.Poly3D[0].Point[0].x) * vv.z;
	vvv.y += (this.Poly3D[0].Point[1].y - this.Poly3D[0].Point[0].y) * vv.x
			+ (this.Poly3D[0].Point[2].y - this.Poly3D[0].Point[0].y) * vv.y
			+ (this.Poly3D[0].Point[3].y - this.Poly3D[0].Point[0].y) * vv.z;
	vvv.z += (this.Poly3D[0].Point[1].z - this.Poly3D[0].Point[0].z) * vv.x
