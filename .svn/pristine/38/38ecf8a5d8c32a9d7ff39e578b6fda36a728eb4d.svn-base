...
public class QiyeQuxuanCommondBo implements ICommond {
	...
	public Page<Record> findlcpw(HashMap<String, Object> map) throws Exception {
		String sql="SELECT c.xzqhdm as xzqhdm,e.qyzzjgdm from fw_user_dep a LEFT JOIN fw_dep b ON a.depid=b.id LEFT JOIN njpt_diqu c ON b.areaid=c.areaid LEFT JOIN fw_user d ON a.userid=d.id LEFT JOIN tz_qiye e ON d.realname=e.qymc WHERE a.userid= ?";
