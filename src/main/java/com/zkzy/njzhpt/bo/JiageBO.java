...
public class JiageBO {
	...
	public Ret savepz(HashMap<String, Object> param) {
		...
		result = Db.save("jg_pinzhong", record);
		if (result) {
			...
			msg.put(Ret.create("message","新增失败,请寻找数据库管理员查看是否是已删除历史品种"));
			return msg;
		}
	}
	...
	public Ret querycjdjg(HashMap<String, Object> map) throws Exception {
		if(map.get("cjdid") == null || "".equals(map.get("cjdid"))){
			map.remove("cjdid");
		}
		if(map.get("year") == null || "".equals(map.get("year"))){
			map.remove("year");
		}
		if(map.get("week") == null || "".equals(map.get("week"))){
			map.remove("week");
		}
	 ...
	 */
	@Before(AutoPageInterceptor.class)
	public Ret querycjdpzjg(HashMap<String, Object> map) throws Exception {
		Object jglxs = map.get("jglxid[]");
		Ret result=new Ret();
		if (jglxs instanceof String) {
			jglxs = new String[] { String.valueOf(jglxs) };
		}
		for(String jglx:(String[])jglxs) {
			map.put("jglxid",jglx);
			Record tbrecord = new Record();
			Record hbrecord = new Record();

			Page<Record> userPageRecord = JiageDAO.querycjdjg(map);

			for (Record userRecord : userPageRecord.getList()) {
				//获取jg_conflix的id
				String conflixid = userRecord.getStr("id");
				//获取填报价格年份
				int year = Integer.parseInt(userRecord.getStr("year"));
				//获取填报价格周数
				int week = Integer.parseInt(userRecord.getStr("week"));
				int qweek = week - 1;
				int qyear = year - 1;

				HashMap<String, Object> param1 = new HashMap<>();
				param1.put("year", qyear);
				param1.put("week", week);
				param1.put("conflixid", conflixid);

				if (JiageDAO.findtbbaojia(param1) == null) {
					tbrecord.set("tbprice", 0);
				} else {
					tbrecord = JiageDAO.findtbbaojia(param1);
				}
				userRecord.setColumns(tbrecord);

				HashMap<String, Object> param2 = new HashMap<>();
				param2.put("year", year);
				param2.put("week", qweek);
				param2.put("conflixid", conflixid);
				if (JiageDAO.findhbbaojia(param2) == null) {
					hbrecord.set("hbprice", 0);
				} else {
					hbrecord = JiageDAO.findhbbaojia(param2);
				}
				userRecord.setColumns(hbrecord);
				result.put(jglx,userPageRecord);
			}
		}
		Ret ret = Ret.create("list", result);
		return ret;
	}
	 ...
	 */
	@Before(AutoPageInterceptor.class)
	public Ret querycjdpzjgapp(HashMap<String, Object> map) throws Exception {
		String jglx = map.get("jglxid").toString();
		Ret result=new Ret();
	 ...
	 * @return
	 */
	public Ret upprice(HashMap<String, Object> param) {
		// TODO Auto-generated method stub
		String sql = "update jg_baojia set price = ? where id = ?";
		int result = Db.update(sql, param.get("price"),param.get("bjid"));
		if (result > 0) {
			Ret msg = Ret.create("ret", "true");
			msg.put(Ret.create("message","修改成功"));
			return msg;
	 ...
	 * @return
	 */
	public boolean savegnbj(HashMap<String, Object> param) {
		// TODO Auto-generated method stub
		Record record = new Record().setColumns(param);
		boolean result = Db.save("jg_baojiagn", "id", record);
		return result;
	 ...
	 * @return
	 */
	public Ret findgnht(HashMap<String, Object> param) {
		int year = Integer.parseInt((String)param.get("year"));
		int week = Integer.parseInt((String)param.get("week"));
		//选中年份上一年用于求同比
		int qyear = year - 1;
		int hyear = year;
		//选中周数上一周用于求环比
		...
		// TODO Auto-generated method stub
		List<Record> userPageRecord = JiageDAO.findgnht(param);
		//循环遍历增加同比和环比
		for(Record userRecord:userPageRecord){
			String pzname = userRecord.getStr("pzname");
			String lslx = userRecord.getStr("lslx");
			//加载去年同期
			HashMap<String, Object> tparam = new HashMap<>();
			tparam.put("year", qyear);
			tparam.put("week", week);
			tparam.put("lslx", lslx);
			tparam.put("pzname", pzname);
			double tprice = 0;
			Record record = null;
			HashMap<String,Object> tb = new HashMap<>();
			if(!JiageDAO.findgnls(tparam).isEmpty()){
				record = JiageDAO.findgnls(tparam).get(0);
			...
			//加载上期
			HashMap<String, Object> hparam = new HashMap<>();
			hparam.put("year", hyear);
			hparam.put("week", qweek);
			hparam.put("lslx", lslx);
			hparam.put("pzname", pzname);
			double hprice = 0;
			if(!JiageDAO.findgnls(hparam).isEmpty()){
				record = JiageDAO.findgnls(hparam).get(0);
	...
	public Ret findguonei(HashMap<String, Object> param) throws Exception {
		...
		for(Record userRecord:userPageRecord.getList()){
			String pzname = userRecord.getStr("pzname");
			String jglx = userRecord.getStr("jglx");
			String lslx = userRecord.getStr("lslx");
			//加载去年同期
			HashMap<String, Object> tparam = new HashMap<>();
			tparam.put("year", qyear);
			tparam.put("week", week);
			tparam.put("jglx", jglx);
			tparam.put("lslx", lslx);
			tparam.put("pzname", pzname);
			tparam.put("page", 1);
			tparam.put("rows", 1);
			...
			Record record = null;
			HashMap<String,Object> tb = new HashMap<>();
			if(!JiageDAO.findguonei(tparam).getList().isEmpty()){
				record = JiageDAO.findguonei(tparam).getList().get(0);
				tprice = record.getDouble("price");
			}
			tb.put("tprice", tprice);
			userRecord.setColumns(tb);
			...
			hparam.put("year", hyear);
			hparam.put("week", qweek);
			hparam.put("jglx", jglx);
			hparam.put("lslx", lslx);
			hparam.put("pzname", pzname);
			hparam.put("page", 1);
			hparam.put("rows", 1);
			double hprice = 0;
			if(!JiageDAO.findguonei(hparam).getList().isEmpty()){
				record = JiageDAO.findguonei(hparam).getList().get(0);
				hprice = record.getDouble("price");
			}
			HashMap<String, Object> hb = new HashMap<>();
			hb.put("hprice", hprice);
	 ...
	 */
	@Before(AutoPageInterceptor.class)
	public Ret querynzjg(HashMap<String, Object> param) throws Exception {
		//获取去年这周的起始日期
		String syear = param.get("year").toString();
		int qyear = Integer.parseInt(syear) - 1;
		String sweek = param.get("week").toString();
		...
		Page<Record> userPageRecord = JiageDAO.querynzjg(param);
		Ret ret = Ret.create("list", userPageRecord);
		return ret;
	}
	 ...
	 * @return
	 */
	public Ret findguonweek(HashMap<String, Object> param) {
		// TODO Auto-generated method stub
		String year = param.get("year").toString();
		String day = param.get("week").toString();
		String sql="select *FROM jg_baojiagn WHERE 1=1 and year=? and week=?";
		Record record=Db.findFirst(sql,year,day);
		if(record!=null){
			Ret ret = Ret.create("list", record);
			return ret;
		}else{
			Ret msg = Ret.create("ret", "false");
			msg.put(Ret.create("message","保存失败"));
			return msg;
		}
	 ...
	 * @return
	 */
	public Ret delprice(HashMap<String, Object> param) {
		// TODO Auto-generated method stub
		boolean result = Db.deleteById("jg_baojia", param.get("bjid"));
		if (result) {
			Ret msg = Ret.create("ret", "true");
			msg.put(Ret.create("message","删除成功"));
			return msg;
