$.extend($.fn.dataTable.defaults, {
	...
	"dom": "t<'col-sm-6'<'pull-left'l><'pull-left'i>><'col-sm-6'p>",
	"language": {
		"paginate": {
			"previous": "上一页",
			"next": "下一页",
	...
	"ajax": function(data, callback, settings) {
		var dt = $(this)[0];
		var queryparam = "page=" + ((data.start/data.length)+1) + "&rows=" + data.length + "&" + dt.dataset.queryparam;
		$.ajax({
			"dataType": 'json',
			"type": "POST",
			"url": dt.dataset.url,
		...
		"bootstrap": {
			"fnInit": function(oSettings, nPaging, fnDraw) {
				...
				$(nPaging).addClass('pagination').append(
					'<ul class="pagination">' + '<li class="prev disabled"><a href="#">' + oLang.sPrevious + '</a></li>' + '<li class="next disabled"><a href="#">' + oLang.sNext + '</a></li>' + '</ul>');
				var els = $('a', nPaging);
				$(els[0]).bind('click.DT', {
					action: "previous"
				}, fnClickHandler);
