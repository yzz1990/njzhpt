...
public class EmergencyBO {
	 ...
	 */
	@Before(AutoPageInterceptor.class)
	public  Ret querytreebumen(HashMap<String, Object> param) throws Exception {
		Page<Record> userPageRecord=EmergencyDAO.querytreebumen(param);
		Ret ret = Ret.create("list", userPageRecord);
		return ret;
	}
	...
	public void updatechildren(Object shangji,Object afterParentNo){
		String splitParent=afterParentNo.toString();
		if(splitParent.endsWith("-")) {
			splitParent=splitParent.substring(0,splitParent.lastIndexOf("-")-1);
		}
		final String finalNo=splitParent;
		Db.find("select * from oa_treelist where shangji=?",shangji).forEach(record1 -> Db.update("oa_treelist", record1.set("parentorderno", finalNo)));
	}
	/**
	 * 修改部门
	 * @param param
	...
	public void addOrUpdateRenYuanOrBuMenMoveOrderNoCursor(String isend,Object needHandleOrderNo,Object type,Object parentNode) {
		...
		if(Db.find("select * from  oa_treelist where isend=? and orderno=? and leixing=? and shangji=?",isend,needHandleOrderNo,type,parentNode).size()!=0) {
			...
			//若是修改的是目录的则需修改对应的子节点的orderNo
			if(isend.equals("0")){

				bigThanNeedHandleOrderNoList.forEach(record -> updatechildren(record.get("id"),record.get("parentorderno")+"-"+record.get("orderno")));
			}
		}
	}
	...
	public String getParentNo(Object orderNo) {
		...
		if(parentNo.endsWith("-")) {
			parentNo=parentNo.substring(0,parentNo.lastIndexOf("-"));
		}
		return parentNo;
	}
