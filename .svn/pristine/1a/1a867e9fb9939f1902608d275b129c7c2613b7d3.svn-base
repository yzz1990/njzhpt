...
public class JiageController extends Controller {
	...
	public void saveprice(){
	    ...
	    String conflixids = param.get("conflixids").toString();
	    String prices = param.get("prices").toString();
	    
	    String[] conflixid = conflixids.split(",");
	    String[] price = prices.split(",");
	    
	    boolean message = true;
	    
	    for(int i = 0; i < conflixid.length; i++){
	    	String tday = matter.format(dt);
		    String id = SqlUtil.uuid();
	    	HashMap<String, Object> map = new HashMap<>();
	    	map.put("year", param.get("year"));
	    	map.put("week", param.get("week"));
	    	map.put("id", id);
	    	map.put("day", tday);
	    	map.put("conflixid", conflixid[i]);
	...
	public void savegnbj() throws ParseException{
		...
		String week = param.get("week").toString();
		String pzs = param.get("pzname").toString();
		String jglxs = param.get("jglx").toString();
		String lslxs = param.get("lslx").toString();
		String prices = param.get("price").toString();
		String data = param.get("data").toString();
		String[] spz = pzs.split(",");
		String[] sjglx = jglxs.split(",");
		String[] slslx = lslxs.split(",");
		String[] sprice = prices.split(",");
		...
		for(int i = 0; i < spz.length; i++){
			...
			map.put("price", price);
			map.put("lslx", lslx);
			boolean result = Duang.duang(JiageBO.class).savegnbj(map);
			res = res && result;
		}
		renderJson(res);
	}
	 ...
	 * 加载国内环比同比价格
	 */
	public void findgnht() throws Exception{
		HashMap<String, Object> param = ParamUtil.getParamMap(this.getParaMap());
		renderJson(Duang.duang(JiageBO.class).findgnht(param).getData().get("list"));
	}
...
//		Db.update(sql,year,week,qyear,qweek);
	 ...
	 * 寻找年周数
	 */
	public void findguonweek() throws Exception{
		HashMap<String, Object> param = ParamUtil.getParamMap(this.getParaMap());
		renderJson(Duang.duang(JiageBO.class).findguonweek(param).getData().get("list"));
	}
}
