...
import com.zkzy.njzhpt.bo.ChuBeiLiangBO;

public class ChuBeiLiangController  extends Controller{
	public void index() {
		Ret ret = null;
		try {
			String method = getAttr("_url_");
			if (StrKit.notBlank(method)) {
				ret = (Ret) ChuBeiLiangBO.class.getMethod(method, HashMap.class)
						.invoke(Duang.duang(ChuBeiLiangBO.class),
