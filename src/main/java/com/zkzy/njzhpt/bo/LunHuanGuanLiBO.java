...
@Before(AutoPageInterceptor.class)
public Ret selectcbjhCH(HashMap<String, Object> map) throws Exception {
	
	Page<Record> list= LunHuanGuanLiDAO.selectcbjhCH(map);
	Ret ret = Ret.create("list", list);
	return ret;
}
@Before(AutoPageInterceptor.class)
...
@Before(AutoPageInterceptor.class)
public Ret findpfhj(HashMap<String, Object> map) throws Exception {
	if("".equals(map.get("qymc").toString())){
		map.remove("qymc");
	}
	if("".equals(map.get("kdbm").toString())){
		map.remove("kdbm");
	}
	if("".equals(map.get("pz").toString())){
		map.remove("pz");
	}
	...
	Page<Record> record = LunHuanGuanLiDAO.findpfhj(map);
...
public Ret validatesqwh(HashMap<String, Object> param1) {
	...
	if(bool){
		...
		if(lhsl){
			...
			/*Map<String, Object> tishi=LunHuanGuanLiDAO.validatedybl(param1);
			if(true==(boolean)tishi.get("ret")){
				ret.put("ret",true).put("message",tishi.get("message"));
			}else{
				ret.put("ret",false);
			}*/
			ret.put("ret",false);
